...
def MSE(pred, true,args):
    gap=(pred - true) ** 2
    column_means = np.mean(gap, axis=0)
    #print(true.shape,gap.shape,column_means.shape)
    '''
    plt.figure()
    plt.plot(column_means, marker='.')
    plt.title(f'{args.seq_len}_{args.pred_len}_per column per step')
    ...
    file_path = os.path.join(directory, f'exp{args.seq_len}_{args.pred_len}_per column per step.png')
    plt.savefig(file_path)
    plt.close()
    '''
    means = np.mean(column_means, axis=1)
    '''
    plt.figure()
    plt.plot(means, marker='.')
    plt.title(f'{args.seq_len}_{args.pred_len}_per step')
    plt.xlabel('Pred Index')
    plt.ylabel('Gap Value')
    plt.grid(True)
    directory = 'pic'
    if not os.path.exists(directory):  
        os.makedirs(directory)
    file_path = os.path.join(directory, f'exp{args.seq_len}_{args.pred_len}_per step.png')
    plt.savefig(file_path)
    plt.close()
    '''
    return np.mean(gap)
...
def RMSE(pred, true,args):
    return np.sqrt(MSE(pred, true,args))
...
def metric(pred, true,args):
    mae = MAE(pred, true)
    mse = MSE(pred, true,args)
    rmse = RMSE(pred, true,args)
    mape = MAPE(pred, true)
    mspe = MSPE(pred, true)
    corr = CORR(pred, true)
