        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, idx, t1, t2, max_lens) in enumerate(test_loader):
            ...
            if True: # i == 20:
                ...
                if i  == 0:
                    ...
                    #print(input.shape)
                    outs1 = pad.DataFrame(true[0, :, :].squeeze())
                    outs2 = pad.DataFrame(pred[0, :, :].squeeze()) 
                    outs3 = pad.DataFrame(input[0, :, :].squeeze()) 
                    outs1.to_excel(f"{self.args.dataset_name}_{self.args.seq_len}_{self.args.pred_len}_true.xlsx", index=False)
                    outs2.to_excel(f"{self.args.dataset_name}_{self.args.seq_len}_{self.args.pred_len}_TimeDiffpred.xlsx", index=False)
                    outs3.to_excel(f"{self.args.dataset_name}_{self.args.seq_len}_{self.args.pred_len}_history.xlsx", index=False)
                    for j in range(self.args.num_vars):
                        gt = np.concatenate((input[0, :, j], true[0, :, j]), axis=0)
                        pd = np.concatenate((input[0, :, j], pred[0, 0,:, j]), axis=0)
                        visual2D(gt, pd, i,j,self.args.seq_len,self.args.pred_len) 
       '''
