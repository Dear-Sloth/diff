...
class Exp_Main(Exp_Basic):
    ...
    def _select_optimizer(self,train_steps):
        ...
        lr_scheduler = optim.lr_scheduler.OneCycleLR(optimizer = model_optim,
    ...
    def train(self, setting):
        ...
        model_optim, lr_scheduler = self._select_optimizer(train_steps)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, idx, t1, t2, max_lens) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        loss = self.model.forward(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                else:
                    if 'SDSB' in self.args.model:
                        loss = self.model.forward(batch_x, batch_x_mark, dec_inp, batch_y_mark, epoch=epoch)
                    else:
                        loss = self.model.forward(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                loss=loss.mean()
            ...
            adjust_learning_rate(model_optim, lr_scheduler, epoch + 1, self.args)
